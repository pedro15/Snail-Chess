...
{
    internal sealed class SEE_Tests
    {   
        private struct SEETest 
        {
            public string fen;
            public string move;
            public int threshold;

            public SEETest(string _fen , string _move, int _threshold)
            {
                fen = _fen;
                move = _move;
        ...
        private Stack<SEETest> tests = new Stack<SEETest>();

        [SetUp]
        public void Setup()
        {
            tests.Clear();
            string[] lines = EPD_DATA.Split('\n' , System.StringSplitOptions.RemoveEmptyEntries);
            for (int i = 0; i < lines.Length; i++)
            {
                string currLine = lines[i];
                string[] contents = currLine.Trim().Split('|' , System.StringSplitOptions.RemoveEmptyEntries);
                if (string.IsNullOrEmpty(currLine) || contents.Length < 3)
                {
                    continue;
                }
                tests.Push(new SEETest(contents[0], contents[1].Replace(" " , string.Empty), int.Parse(contents[2])));
            }
        }

        [Test]
        public void SEETestSuite()
        {
            SearchEngine searchEngine = new SearchEngine();
            BotPersonality personality = BotPersonality.MAX;
            searchEngine.AINewGame();
            searchEngine.AILoadParams(in personality.searchOptions, in personality.evaluationParams);
            ...
            while(tests.Count > 0)
            {
                SEETest currTest = tests.Pop();
                
                pos = Notation.ParseFEN(currTest.fen);
                searchEngine.AILoadPosition(pos);
                
                legal_moves = Move.GetLegalMoves(in pos);
                loaded_move = Move.FromLAN(currTest.move, in legal_moves);

                bool see_result = searchEngine.SEE(in loaded_move, currTest.threshold);
                Assert.That(see_result, Is.EqualTo(true) , $"fen: '{currTest.fen}' | move: '{currTest.move}' loaded_move: {loaded_move} | threshold: {currTest.threshold}");
            }
        }

        const string EPD_DATA = @"
