...
{
    public static class PerftSuiteFactory 
    {
        public static readonly PerftSuiteTest[] suiteTests; 

        static PerftSuiteFactory()
        {
            string[] lines = EPD_DATA.Split("\n"), line_contents = null , tokens = null;
            List<PerftSuiteTest> valid_tests = new List<PerftSuiteTest>();
            List<System.Tuple<int,ulong>> c_expected = new List<System.Tuple<int, ulong>>();
            string current_line = string.Empty , fen = string.Empty;
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                fen = string.Empty;
                c_expected.Clear();
                line_contents = current_line.Split(";");

                for (int j = 0; j < line_contents.Length; j++)
                {
                    if(line_contents[j].Contains("D"))
                    {
                        tokens = line_contents[j].Replace("D" , string.Empty).Split(' ' , System.StringSplitOptions.RemoveEmptyEntries);
                        if (tokens.Length != 2) continue;
                        c_expected.Add(new System.Tuple<int, ulong>(int.Parse(tokens[0]) , ulong.Parse(tokens[1])));
                    }else 
                    {
                        fen = line_contents[j];
                    }
                ...
                {
                    valid_tests.Add(new PerftSuiteTest(fen, c_expected.ToArray()));
                }
            }

            suiteTests = valid_tests.ToArray();
        }

        public static void Init() { }
    
        private const string EPD_DATA = @"
